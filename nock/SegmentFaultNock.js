...
(function () {
	...
	const features = {
		...
		hidden_first_login_dialog: {
			enabled: GM_getValue("hidden_first_login_dialog", true),
		},
	};

	const setModal = `<div class="nock-modal-dialog"> <div class="modal-setting" onClick="event.cancelBubble = true"> <div class="modal-header"> <h3>功能设置</h3> <span class="btn-dialog-close">×</span> </div> <div class="modal-body"> <div class="setting-item"> <span> 文章显示时间优化 </span> <span> <input type="checkbox" id="feature-mark-datetime" aria-nock="datetime" /> <label for="feature-mark-datetime"></label> </span> </div> <div class="setting-item"> <span> 显示历史修改</span> <span> <input type="checkbox" id="feature-mark-modified-history" aria-nock="modified_history" /> <label for="feature-mark-modified-history"></label> </span> </div> <div class="setting-item"> <span> 定位文字调整 </span> <span> <input type="checkbox" id="feature-mark-adjust-position" aria-nock="adjust_position" /> <label for="feature-mark-adjust-position"></label> </span> </div> <div class="setting-item"> <span> 点击文章阅读时长跳转文章底部 </span> <span> <input type="checkbox" id="feature-mark-readtime" aria-nock="readtime" /> <label for="feature-mark-readtime"></label> </span> </div> <div class="setting-item"> <span> 首次访问不显示登录 </span> <span> <input type="checkbox" id="feature-hidden-first-login-dialog" aria-nock="hidden_first_login_dialog" /> <label for="feature-hidden-first-login-dialog"></label> </span> </div> </div> </div> </div>`;
	...
	if (features.hidden_first_login_dialog.enabled) {
		document.cookie = "showLogin=true";
	}
