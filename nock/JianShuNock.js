...
(function () {
	...
	if (features.mark.datetime.enabled) {
		...
		createObserver((observer) => {
			...
			const formattedPostTime = `${postTime.innerText}（${rawPostTimeAgo}）`;
			const formattedUpdateTime = `${updateTime.innerText}（${rawUpdateTimeAgo}）`;
	...
	if (features.mark.readtime.enabled) {
		createObserver((observer) => {
			...
			const textCount = parseInt(wordCountItem.innerText.match(/\d+/g));
